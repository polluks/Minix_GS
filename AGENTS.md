...
- **MVN (verified with vasm)**: `mvn #$dst,#$src` emits bytes `54 <src> <dst>` — vasm SWAPS the two operands, and the CPU reads the source bank from the first byte. Semantics: copy A+1 bytes from src bank at Y to dst bank at X (banks come from the operands, not DB). `mvn #$02,#$00` → `54 00 02` copies bank 0 → bank $02.
