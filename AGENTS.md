...
- **Interrupt bring-up (RECONSIDERED — emulator changes REVERTED)**: the GSSquared clone changes were reverted per upstream owner (jawaidbazyar2, PR #158): `f_vbl_enable` is ALREADY bit 3 of `f_INTEN` (they are a union), so storing INTEN already enables Mega II VBL IRQs; the level-sensitive `update_megaii_interrupt(ds,true)` on the INTEN write was the real (wrong) behavior change — irqtest requires IRQ NOT to fire on the enable write, only on the next VBL edge. And `vp_read` intentionally pulls $FFE4-$FFFF from ROM when IOLC shadowing is on (Firmware Reference + TN #68), regardless of the LC soft switches — 8-bit software goes through the ROM trampoline at $C074 / $E1:0010 rather than any LC-RAM vector at $FFFE. So the kernel's LC-RAM-vector approach (writing $FFE4/$FFE6) never fires on stock GSSquared: **use the ROM trampoline path instead** (patch the ROM dispatcher targets or hook via $C074-style indirection), and write INTEN/$C041 and just wait for the next VBL edge. The old "60 Hz jiffies ticker" milestone depended on the reverted behavior and must be revalidated with the ROM-trampoline approach.
