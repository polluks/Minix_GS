#!/usr/bin/env python3
"""BP at all boot-block addresses to map the exact flow after the SmartPort call."""
...
def main():
    ...
    try:
        ...
        for addr in (0x0801, 0x0806, 0x0809, 0x080B, 0x080E, 0x0810, 0x0816, 0x083C):
            gs.bp(addr)
        print("8 BPs set")
        ...
        while time.time() < deadline:
            ...
            if eid == 1:
                r = gs.regs()
                hits.append(r)
                print("STOP %02X:%04X p=%02X C=%d a=%04X x=%04X y=%04X sp=%04X db=%02X"
                      % (r['pb'], r['pc'], r['p'], r['p'] & 1, r['a'], r['x'], r['y'], r['sp'], r['db']))
                gs.req(CONTINUE)
                if len(hits) > 40:
                    break
        print("hits:", ["%02X:%04X" % (r['pb'], r['pc']) for r in hits[:20]])
        gs.req(QUIT)
    finally:
        time.sleep(0.5)
        try: proc.terminate()
